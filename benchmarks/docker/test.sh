...
./docker_build.sh && docker run -it --rm \
  ...
  -v "/tmp/:/tmp/:rw" \
  --network=host \
  --env NH_NH_DOCKER_IMAGE="envoyproxy/nighthawk-dev:latest" \
  ...
  --env ENVOY_IP_TEST_VERSIONS=v4only oschaaf/benchmark-dev:latest \
  python3 ./benchmarks --log-cli-level=debug -vvvv

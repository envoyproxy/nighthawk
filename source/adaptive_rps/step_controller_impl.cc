...
using nighthawk::adaptive_rps::UNKNOWN_HEALTH;
...
double TotalWeightedScore(const BenchmarkResult& benchmark_result) {
  ...
  for (const MetricEvaluation& evaluation : benchmark_result.metric_evaluations()) {
    if (evaluation.health_status() == UNKNOWN_HEALTH) {
      score += evaluation.health_score() * evaluation.metric_spec().weight();
    } else {
      score +=
          (evaluation.health_status() == HEALTHY ? 1.0 : -1.0) * evaluation.metric_spec().weight();
    }
